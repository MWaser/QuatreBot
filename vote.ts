...
var vote = function (i: number, author: string, identifier: string, weight: number) {
    steem.broadcast.vote(voters[i].wif, voters[i].name, author, identifier, weight, function (err, result) {
        if (err == null) console.log(voters[i].name + " voted! ");
        ...
        if (++i < voters.length) { vote(i, author, identifier, 1); } else process.exit();
    });
}
